...
for(int i=2;i<=n;i++){
	for(int j=1;j<=m;j++){
 		int pre_max_cost=INT_MIN;

 		if(j>1)pre_max_cost=max(pre_max_cost,dp[i-1][j-1]);
 		if(j<m)pre_max_cost=max(pre_max_cost,dp[i-1][j+1]);
 		pre_max_cost=max(pre_max_cost,dp[i-1][j]);

 		dp[i][j]=grid[i][j]+pre_max_cost;
	}
}
